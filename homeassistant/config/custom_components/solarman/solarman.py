...
class Inverter:
    ...
    def get_statistics(self):
        log.debug(f"----------------------------------PASSOU NO GET STATISTICS -------------------------------------")
        result = 1
        ...
        try:
            log.debug(f"try no statistics")
            for request in requests:
                log.debug(f"no for do statistisc")
                start = request['start']
                end = request['end']
                ...
                while attempts_left > 0:
                    ...
                    try:
                        ...
                        log.debug(f"conectou ao servidor")
